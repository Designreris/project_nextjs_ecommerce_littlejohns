...
export default function Header() {
  ...
  return (
    <header className="bg-clr_light relative w-full h-[50px] flex justify-between items-center">
      <div
        className={`flex items-center justify-center ml-4 hover:cursor-pointer text-clr_dark hover:text-clr_background`}
      ...
      <ul
        ...
        } absolute top-[50px] text-balance text-center w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 bg-clr_primary`}
      >
        {headerMenuData !== null ? (
          headerMenuData.map((item) => (
            <li key={item} className="m-4">
              <Link
                href={`#${item}`}
                onClick={() => handleMenuBtn()}
                className="w-full font-medium text-clr_dark hover:text-clr_background"
              >
                {item}
              </Link>
            </li>
